...
pub async fn run(event_loop: EventLoop<()>, window: Window, inputs: Inputs<'_>, num_vertices: u32) {
    ...
    // creating a rendered surface for wininit window to provide drawing for the platform supported by wininit
    ...
    let shader = device.create_shader_module(
        &(wgpu::ShaderModuleDescriptor { label: None, source: inputs.source })
    );
